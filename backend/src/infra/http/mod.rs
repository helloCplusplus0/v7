...
impl<T: Serialize> IntoResponse for HttpResponse<T> {
    fn into_response(self) -> Response {
        let status = HttpStatusCode::from_u16(self.status).unwrap_or(HttpStatusCode::INTERNAL_SERVER_ERROR);
...
pub trait HttpClient: Send + Sync {
    ...
    /// 设置默认超时时间
    fn with_timeout(self, timeout_seconds: u64) -> Self where Self: Sized;
    
    /// 添加默认请求头
    fn with_header(self, key: &str, value: &str) -> Self where Self: Sized;
}
...
impl HttpClient for SimpleHttpClient {
    async fn get(&self, url: &str) -> Result<String> {
        ...
        Ok(format!("{{\"url\": \"{}\", \"method\": \"GET\"}}", url))
    }
    
    async fn post(&self, url: &str, body: &str) -> Result<String> {
        tracing::info!("模拟POST请求: {} (body: {})", url, body);
        Ok(format!("{{\"url\": \"{}\", \"method\": \"POST\", \"body\": \"{}\"}}", url, body))
    }
    
    async fn put(&self, url: &str, body: &str) -> Result<String> {
        tracing::info!("模拟PUT请求: {} (body: {})", url, body);
        Ok(format!("{{\"url\": \"{}\", \"method\": \"PUT\", \"body\": \"{}\"}}", url, body))
    }
    
    async fn delete(&self, url: &str) -> Result<String> {
        tracing::info!("模拟DELETE请求: {}", url);
        Ok(format!("{{\"url\": \"{}\", \"method\": \"DELETE\"}}", url))
    }
