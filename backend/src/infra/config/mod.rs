...
pub struct Config {
    ...
    watchers: RwLock<Vec<Box<dyn Fn(&str, &ConfigValue) + Send + Sync>>>,
}
...
impl Config {
    ...
    pub fn get(&self, key: &str) -> Option<ConfigValue> {
        ...
        if let Ok(env_value) = std::env::var(key) {
            ...
            // 缓存结果
            self.values.write().unwrap().insert(key.to_string(), config_value.clone());
    ...
    pub fn host(&self) -> String {
        self.get_string_or("HOST", if self.environment.is_production() { "0.0.0.0" } else { "127.0.0.1" })
    }
    ...
    pub fn log_level(&self) -> String {
        self.get_string_or("LOG_LEVEL", match self.environment {
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        if self.environment.is_production() {
            if self.get_string("JWT_SECRET").is_none() {
                return Err(AppError::validation("生产环境必须设置 JWT_SECRET"));
            }
            
            if self.get_string("DATABASE_URL").is_none() {
                return Err(AppError::validation("生产环境必须设置 DATABASE_URL"));
            }
        }
        ...
        let port = self.port();
        if port == 0 {
            return Err(AppError::validation(format!("无效的端口号: {}", port)));
        }
...
/// 全局配置实例
static GLOBAL_CONFIG: std::sync::LazyLock<Config> = 
    std::sync::LazyLock::new(|| Config::from_env());
...
macro_rules! require_config {
    ($key:expr) => {
        crate::infra::config::config()
            .get_string($key)
            .ok_or_else(|| crate::core::error::AppError::validation(format!("缺少必需的配置: {}", $key)))?
    };
} 