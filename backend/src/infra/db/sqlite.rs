...
impl SqliteDatabase {
    ...
    fn execute_query_internal(&self, sql: &str, params: &[&str]) -> Result<Vec<DbRow>> {
        ...
        let rows = stmt
            .query_map(params_from_iter(params), Self::row_to_dbrow)
            .map_err(|e| AppError::database(format!("查询执行失败: {}", e)))?;

        let mut result = Vec::new();
        for row_result in rows {
            let row =
                row_result.map_err(|e| AppError::database(format!("行数据解析失败: {}", e)))?;
            result.push(row);
        }
...
impl Transaction for SqliteTransaction {
    async fn query(&self, sql: &str, params: &[&str]) -> Result<Vec<DbRow>> {
        ...
        let rows = stmt
            .query_map(params_from_iter(params), SqliteDatabase::row_to_dbrow)
            .map_err(|e| AppError::database(format!("查询执行失败: {}", e)))?;
