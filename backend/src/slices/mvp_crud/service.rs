...
use super::types::{
    ...
    DeleteItemResponse, ListItemsQuery, ListItemsResponse, CrudError, CrudResult
};
    ...
    C: Cache + JsonCache + Clone,
{
    async fn create_item(&self, req: CreateItemRequest) -> CrudResult<CreateItemResponse> {
        ...
        metrics().lock().unwrap().as_ref().unwrap().record_timer("crud.create_item", duration);
    ...
    async fn get_item(&self, id: &str) -> CrudResult<GetItemResponse> {
        ...
        if let Ok(Some(item)) = self.cache.get_json::<Item>(&cache_key).await {
            let duration = timer.stop();
            metrics().lock().unwrap().as_ref().unwrap().record_timer("crud.get_item_cache", duration);
        ...
        match self.repository.find_by_id(id).await? {
            Some(item) => {
                ...
                let duration = timer.stop();
                metrics().lock().unwrap().as_ref().unwrap().record_timer("crud.get_item_db", duration);
    ...
    async fn update_item(&self, id: &str, req: UpdateItemRequest) -> CrudResult<UpdateItemResponse> {
        ...
        // 获取现有项目
        let mut item = match self.repository.find_by_id(id).await? {
            Some(item) => item,
            None => return Err(CrudError::ItemNotFound { id: id.to_string() })
        };
        ...
        if let Some(new_name) = &req.name {
            if new_name != &item.name {
                if let Some(_existing) = self.repository.find_by_name(new_name).await? {
                    return Err(CrudError::ItemNameExists { name: new_name.clone() });
                }
            }
        }
        ...
        let duration = timer.stop();
        metrics().lock().unwrap().as_ref().unwrap().record_timer("crud.update_item", duration);
    ...
    async fn delete_item(&self, id: &str) -> CrudResult<DeleteItemResponse> {
        ...
        // 记录日志
        let log_entry = LogEntry::new(LogLevel::Info, format!("删除项目成功: {}", id))
            .with_field("item_id", id);
        logger().lock().unwrap().log(log_entry);
        
        let duration = timer.stop();
        metrics().lock().unwrap().as_ref().unwrap().record_timer("crud.delete_item", duration);
    ...
    async fn list_items(&self, query: ListItemsQuery) -> CrudResult<ListItemsResponse> {
        ...
        let duration = timer.stop();
        metrics().lock().unwrap().as_ref().unwrap().record_timer("crud.list_items", duration);
    ...
    D: Database + Clone,
{
    ...
    /// 将数据库行转换为Item
    fn row_to_item(&self, row: &DbRow) -> CrudResult<Item> {
        let id = row.get("id")
            .and_then(|v| v.as_str())
            .ok_or_else(|| CrudError::Database { message: "缺少id字段".to_string() })?
            .to_string();
            
        let name = row.get("name")
            .and_then(|v| v.as_str())
            .ok_or_else(|| CrudError::Database { message: "缺少name字段".to_string() })?
            .to_string();
        ...
        let value = row.get("value")
            .and_then(|v| v.as_i64())
            .ok_or_else(|| CrudError::Database { message: "缺少value字段".to_string() })? as i32;
            
        let created_at = row.get("created_at")
            .and_then(|v| v.as_str())
            .and_then(|s| s.parse().ok())
            .ok_or_else(|| CrudError::Database { message: "无效的created_at字段".to_string() })?;
            
        let updated_at = row.get("updated_at")
            .and_then(|v| v.as_str())
            .and_then(|s| s.parse().ok())
            .ok_or_else(|| CrudError::Database { message: "无效的updated_at字段".to_string() })?;
    ...
    D: Database + Clone,
{
    ...
    async fn update(&self, item: &Item) -> CrudResult<()> {
        ...
        if affected_rows == 0 {
            return Err(CrudError::ItemNotFound { id: item.id.clone() });
        }
    ...
    async fn delete(&self, id: &str) -> CrudResult<bool> {
        ...
        let affected_rows = self.db.execute(sql, &[id]).await
            .map_err(|e| CrudError::Database { 
    ...
    async fn list(&self, limit: u32, offset: u32, sort_by: Option<&str>, desc: bool) -> CrudResult<(Vec<Item>, u32)> {
        ...
        // 安全的SQL构建：白名单验证 + 参数化查询
        let sql = format!(
            "SELECT id, name, description, value, created_at, updated_at FROM items ORDER BY {} {} LIMIT ? OFFSET ?",
            sort_column, order
        );
        ...
        let rows = self.db.query(&sql, &params).await
            .map_err(|e| CrudError::Database { 
                message: format!("查询项目列表失败: {} - SQL: {}", e, sql) 
            })?;
