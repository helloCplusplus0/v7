...
mod tests {
    ...
    async fn create_test_service() -> ConcreteCrudService {
        ...
        // ✅ 关键修复：初始化数据库表结构
        repository.init_table().await.expect("Failed to initialize database table");
    ...
    fn create_test_request(name: &str, value: i32) -> CreateItemRequest {
        CreateItemRequest {
            name: name.to_string(),
            description: Some(format!("{} 的描述", name)),
            value,
        }
    }
    ...
    async fn test_create_item_success() {
        ...
        assert!(result.is_ok(), "创建项目应该成功，但失败了: {:?}", result.err());
    ...
    async fn test_list_items_with_data() {
        ...
        for (name, value) in items_to_create {
            ...
            assert!(result.is_ok(), "创建项目 {} 应该成功", name);
        }
